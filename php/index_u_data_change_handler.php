...
switch ($header) {
	...
	case 'verificarLabPrograma':
		actionRestriction_103();
		$postValues_1 = encryptControl('decrypt', (mysql_real_escape_string(stripslashes(clean($_POST['labid'])))), $_SESSION['qap_token']);
		$postValues_2 = mysql_real_escape_string(stripslashes(clean($_POST['programid'])));
		$qry = "SELECT 1
				FROM configuracion_laboratorio_analito
				WHERE id_laboratorio = $postValues_1
				AND id_programa = $postValues_2
				LIMIT 1";

		$rs = mysql_query($qry);
		mysqlException(mysql_error(), $header . "_ENRCHK");

		$enrolamientoRealizado = (mysql_num_rows($rs) > 0) ? 1 : 0;

		if ($enrolamientoRealizado == 1) {
			echo '<response code="0">ENROLAMIENTO_BLOQUEADO</response>';
		} else {
			echo '<response code="1">OK</response>';
		}
		break;
